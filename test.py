        ...
        for (x, y, w, h) in features:
            ...
            if confidence > 70 and not justscanned:
                ...
                if int(cnt) == 30:
                    ...
                    mycursor.execute("SELECT accs_id FROM accs_hist WHERE accs_prsn = %s AND accs_date = %s AND check_out_time IS NULL", (pnbr, date.today()))
                    existing = mycursor.fetchone()
                    
                    if existing:
                        # Update check-out time
                        mycursor.execute("UPDATE accs_hist SET check_out_time = NOW() WHERE accs_id = %s", (existing[0],))
                    else:
                        # Insert new check-in record
                        mycursor.execute("INSERT INTO accs_hist (accs_date, accs_prsn, check_in_time) VALUES (%s, %s, NOW())", 
                                       (date.today(), pnbr))
...
def checkin():
    ...
    # Get personnel number from session or user data
    mycursor.execute("SELECT prs_nbr FROM prs_mstr WHERE prs_name = %s", (session['username'],))
    pnbr = mycursor.fetchone()[0]
    
    # Check if already checked in
    mycursor.execute("SELECT accs_id FROM accs_hist WHERE accs_prsn = %s AND accs_date = %s AND check_out_time IS NULL", 
                    (pnbr, date.today()))
    existing = mycursor.fetchone()
...
def checkout():
    ...
    if not existing:
        flash('Anda belum check-in hari ini', 'warning')
    else:
        mycursor.execute("UPDATE accs_hist SET check_out_time = NOW() WHERE accs_id = %s", 
                       (existing[0],))
        mydb.commit()
